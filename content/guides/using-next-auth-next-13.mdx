...
### Register API: Go back to api folder. Inside the api folder create another folder called register inside that route.ts
...
### Functionality: Then go back to AuthForm and makes a POST request to the "/api/register" endpoint using the axios library inside the onSubmit handler.

For the code, follow the video or check Antonio's repo.

> Verification: Go to your auth form in the browser and register a new user. If everything is working properly a new user should be created in your database.
...
### <ins>Login with Credentials</ins>
...
### <ins>Github Environment Variables</ins>
...
### <ins>Google Environment Variables</ins>
